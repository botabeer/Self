...
━━━━━━━━━━━━━━━"""
        ...
        if token:
            token_text = f"""🔑 معلومات التوكن:
━━━━━━━━━━━━━━━
⚠️ تحذير: لا تشارك هذا التوكن مع أحد!

📱 التوكن الخاص بك:
{token}

━━━━━━━━━━━━━━━
💡 هذا التوكن يمنح الوصول الكامل لحسابك
🔒 احتفظ به في مكان آمن
━━━━━━━━━━━━━━━"""
            
            # إرسال التوكن في رسالة خاصة أفضل من المجموعة
            try:
                cl.send_message(s, token_text)
                send(g, "✅ تم إرسال التوكن في رسالة خاصة لك")
            except:
                # إذا فشل الإرسال الخاص، أرسله في المجموعة
                send(g, token_text)
        else:
            send(g, "❌ فشل الحصول على التوكن")
    
    elif cmd == "savetoken":
        # حفظ التوكن في ملف نصي للمالك فقط
        if not is_owner(s):
            send(g, "⛔ هذا الأمر للمالك فقط!")
            return
        ...
        if token:
            try:
                with open("my_token.txt", "w", encoding="utf-8") as f:
                    ...
                    f.write(f"تاريخ الحفظ: {datetime.now()}\n")
                    f.write(f"━━━━━━━━━━━━━━━━━━━━\n\n")
                    f.write(f"Token: {token}\n\n")
                    f.write(f"⚠️ تحذير: لا تشارك هذا الملف مع أحد!\n")
                
                send(g, "✅ تم حفظ التوكن في ملف my_token.txt على السيرفر")
            except Exception as e:
                send(g, f"❌ فشل حفظ التوكن: {e}")
        else:
            send(g, "❌ فشل الحصول على التوكن")
